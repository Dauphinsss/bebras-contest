...
Actualización de fase 6: estas cinco tareas se integraron al catálogo verificado de 43 que llegó en paralelo por `origin/master`. Ese catálogo importa como opción múltiple en cuarentena las siete que no se pueden derivar del PDF (04, 09, 11, 19, 31, 37 y 40); aquí se reemplazan por su versión redactada a mano y salen de cuarentena. Conservan los metadatos verificados —id canónico, título, país, año, `sourceTaskCode`, dificultades— y sus bloques de explicación deterministas; solo cambian enunciado, consigna y respuesta. La 19 pasa a huecos sobre su mismo id, después de comprobar que no tenía ninguna respuesta registrada. El validador del catálogo exige que cada una de esas siete esté o en cuarentena como opción múltiple, o redactada con tipo interactivo y disponible: nunca las dos cosas ni ninguna, así que regenerar la migración sin reponerlas falla en vez de degradarlas en silencio. `bun run db:tasks` ya inserta solo lo que falta sin tocar lo existente, así que los scripts de instalación propios dejaron de hacer falta. Las cinco tareas se recorren además en desafíos reales: como cada categoría Bebras tiene su rango de edad y un desafío solo admite tareas con dificultad para ese rango, hacen falta dos, Titi (09 y 19) y Kuntur (31, 37 y 40).
